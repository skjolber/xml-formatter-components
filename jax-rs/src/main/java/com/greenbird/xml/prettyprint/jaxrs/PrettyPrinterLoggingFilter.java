...
public class PrettyPrinterLoggingFilter implements ContainerRequestFilter, ContainerResponseFilter, WriterInterceptor {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(PrettyPrinterLoggingFilter.class);
	
	public static final String CACHE_STREAM = CacheStream.class.getName();
    public static final String LOG_BUILDER = PrettyPrinterLoggingFilter.class.getName() + ":builder";
    public static final String LOG_PRETTY_PRINTER = PrettyPrinterLoggingFilter.class.getName() + ":prettyPrinter";

	private PrettyPrinterAnnotationFactory factory = new PrettyPrinterAnnotationFactory();
    ...
    public PrettyPrinterLoggingFilter() {
        this(LOGGER);
    }
    
	public PrettyPrinterLoggingFilter(Logger logger) {
		this.logger = logger;
	}
	...
	public void filter(ContainerRequestContext requestContext) throws IOException {
		...
		if(requestContext.hasEntity()) {
			PrettyPrinter prettyPrinter = getPrettyPrinter(requestContext);

			requestContext.setProperty(LOG_PRETTY_PRINTER, prettyPrinter);
	...
	private PrettyPrinter getPrettyPrinter(ContainerRequestContext requestContext) {
		PrettyPrint methodAnnotation = resourceInfo.getResourceMethod().getAnnotation(PrettyPrint.class);
		
		// method overrides the class annotation in full
		if(methodAnnotation != null) {
			return factory.getPrettyPrinter(methodAnnotation);
		} 
		
		PrettyPrint classAnnotation = resourceInfo.getResourceClass().getAnnotation(PrettyPrint.class);

		return factory.getPrettyPrinter(classAnnotation);
	}
	...
	public void aroundWriteTo(WriterInterceptorContext context) throws IOException, WebApplicationException {
        ...
        if (stream != null && buffer != null) {
        	...
        	PrettyPrinter prettyPrinter = (PrettyPrinter) context.getProperty(LOG_PRETTY_PRINTER);
        	if(prettyPrinter == null) {
	    		prettyPrinter = getPrettyPrinter(context.getAnnotations());
        	}
	...
	private PrettyPrinter getPrettyPrinter(Annotation[] annotations) {
		PrettyPrint prettyPrint = null;
		if(annotations != null) {
			for(Annotation annotation : annotations) {
				if(annotation instanceof PrettyPrint) {
					prettyPrint = (PrettyPrint)annotation;
		...
		return factory.getPrettyPrinter(prettyPrint);
	}
